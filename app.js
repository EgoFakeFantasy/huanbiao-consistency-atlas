...
const tiers = [
  ...
  {name:'弱紧致—Ramsey 基数',code:'TIER 04',summary:'从弱紧致、不可言喻、remarkable 和 Erdős 参数层到 Jónsson／Rowbottom／Ramsey 的等一致组；分清同一基数上的蕴含与存在断言的强度。',route:'弱紧致 → 不可言喻 → remarkable → ω-Erdős ⇢ 0♯ → ω₁-Erdős ⇢ Jónsson／Rowbottom／Ramsey',relations:[['已证等一致','存在弱紧致基数 ≡Con ℵ₂ 具有树性质',references.mitchell],['已证等一致','存在 remarkable ≡Con L(ℝ) 在 proper forcing 下的绝对性',references.schindlerremarkable],['已证等一致','存在 Jónsson ≡Con 存在 Rowbottom ≡Con 存在 Ramsey',references.mitchellramsey],['同一基数','Ramsey ⇒ Rowbottom ⇒ Jónsson；反向不能由等一致性推出',references.kleinberg],['Erdős 参数','ω-Erdős 可与 V=L 共存；ω₁-Erdős ⇒ 0♯ 存在',references.rowbottom],['开放问题','奇异基数的后继能否为 Jónsson？ZFC 下 ℵω 能否为 Rowbottom？',references.rinotjonsson]],bands:[
    ...
    {label:'0♯ 层',theories:[['ZFC + 0♯ 存在','L 的不可辨元'],['ZFC + 无参数解析决定性 Π¹₁-Det','Martin–Harrington 定理']],reason:'解析决定性是细体、无实参数的配对。0♯ 可在 L 中给出每个可数 α 的 α-Erdős 基数；¬SCH 必有 0♯，但其精确强度更高。与上一框的连接线不主张已核对的严格强度。',sources:[references.harrington,references.larson,references.mitchellcover],problems:['analytic','schsharp'],link:'dashed'},
